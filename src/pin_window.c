...
PinWindow* pin_window_create(PinWindowCallbacks callbacks) {
  ...
  if (pin_window) {
    ...
    if (pin_window->window) {
      ...
      text_layer_set_text(pin_window->main_text, "Enter a stop number");
      ...
      for (int i = 0; i < PIN_WINDOW_NUM_CELLS; i++) {
        selection_layer_set_cell_width(pin_window->selection, i, 24);
      }
      selection_layer_set_cell_padding(pin_window->selection, 4);
      selection_layer_set_active_bg_color(pin_window->selection, PBL_IF_COLOR_ELSE(GColorVividCerulean, GColorBlack));
      selection_layer_set_inactive_bg_color(pin_window->selection, GColorLightGray);
